...
- Objective: Реализовать первую клиентскую интерактивность главной страницы. **Часть (1) — решена,
  ...
  `?department=<id>`. OQ-2 включает три равноправных варианта (см. "Open questions" ниже, включая
  замечание о том, как каждый вариант соотносится с текстом `docs/05`/Step 5 — не как аргумент в
  пользу одного из них): (a) Step 4 реализует полный набор `opening/active/switching/closing` с
  минимальным UI; (b) Step 4 реализует только упрощённый бинарный select/close, без под-состояний;
  (c) Step 4 не реализует выбор отдела вовсе — ограничивается `hero ↔ overview`, а весь
  `SELECT_DEPARTMENT`/`CLOSE_DEPARTMENT`/URL-sync целиком переносится в Step 5. Ни один из трёх
  ...
  `OutcomePanel`, mobile touch flow, GSAP-анимации — вне scope в любом случае (Step 5/6/7 по уже
  утверждённому 8-шаговому скелету). Диагностика/`contact-open` — вне scope не только Step 4, а
  **всего текущего 8-шагового milestone** целиком (см. уже утверждённые записи Step 3, "вне scope
  всего текущего 8-шагового milestone"; `docs/13` Этап 8 — отдельная будущая макро-фаза, не один из
  Steps 1–8).
- In scope — безусловно, не зависит от OQ-1/OQ-2/OQ-3 (реализуется при любом ответе, включая
  ...
  - **URL при boot (безусловно, минимальная часть):** при валидном `?department=<id>` в query
    string при первой загрузке не показывать голый `hero`-без-раскрытия (`docs/05`: "...чтобы
    шаринг ссылки на конкретный отдел не требовал лишнего клика") — как минимум сразу показывается
    раскрытый `overview`.
...
- Out of scope:
  ...
  - Диагностика (`OPEN_DIAGNOSTIC`, `diagnostic-*`), `contact-open` — **вне scope всего текущего
    ...
    ("вне scope всего текущего 8-шагового milestone"). Появляются только в `docs/13` Этап 8 —
    отдельной будущей макро-фазе, не одном из Steps 1–8.
...
- Skeptic verdict (review плана, Phase A): **round 1** `FAIL` (`b8cecb6` — OQ-2 скрыто предполагал
  `SELECT_DEPARTMENT` в Step 4 без варианта "вообще не в Step 4") → **round 2** `FAIL` (`0345e47` —
  устаревшие ссылки на acceptance criteria в Risks после переразметки; асимметричный тон OQ-2) →
  **round 3** `FAIL` (`8690a71` — manual check и unit-тесты bullet противоречили сами себе;
  Objective всё ещё асимметрично выделял вариант (c)) → **round 4** `FAIL` (`2c77a12` — Risks
  ошибочно относил решение о GSAP к OQ-1, хотя GSAP уже безусловно исключён; 2 minor ссылки) →
  **round 5** `FAIL` (`c129c42` — диагностика ошибочно сгруппирована со Step 7/8, хотя она вне
  всего 8-шагового milestone; OQ-2 всё ещё асимметричен; неточная цитата docs/05; OQ-3 не отмечал
  зависимость от OQ-2) → **round 6** `PASS` (`94a0ebc`, финальный для Phase A) — все находки round
  5 подтверждены устранёнными по существу; 2 Minor (терминологический дрейф "вне scope" vs "вне";
  остаточная лёгкая асимметрия деталей в закрывающем замечании OQ-2) не блокируют предъявление
  плана пользователю.
- Skeptic findings (round 6, финальные): Blocker/Critical/Major — нет. Minor: (1) фраза "вне всего
  текущего 8-шагового milestone" не дословно совпадала с формулировкой Step 3 "вне **scope** всего
  текущего 8-шагового milestone" — исправлено точечно после review, до коммита финальной версии;
  (2) закрывающее замечание OQ-2 даёт варианту (b) единственное слово "промежуточный" вместо
  развёрнутого пояснения, как у (a)/(c) — признано законной документальной ремаркой с адекватной
  оговоркой о ненормативности, не скрытым подталкиванием к ответу.
- Completion evidence: `WORKPLAN.md` (эта секция, 6 раундов правок, коммиты `b8cecb6`…`94a0ebc`);
  план ещё не реализован — Completion evidence по исполнению будет добавлено после Phase B.
