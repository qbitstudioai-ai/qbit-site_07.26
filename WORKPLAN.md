...
- Objective: Реализовать первую клиентскую интерактивность главной страницы: (1) переход
  `hero → overview` по клику любой CTA (`ACTIVATE_CTA`) — при включённом JavaScript пять отделов по
  умолчанию скрыты и раскрываются только после клика; без JavaScript ничего не меняется относительно
  уже реализованного и протестированного Step 3 (все пять отделов видны и доступны с клавиатуры
  сразу); (2) явное состояние выбора отдела (`SELECT_DEPARTMENT`/`CLOSE_DEPARTMENT`/`ESCAPE`) с
  двусторонней синхронизацией `?department=<id>`, в границах, зафиксированных решением по **OQ-2**
  (сколько именно из `department-opening/active/switching/closing` реализуется в этом шаге, а не в
  Step 5, решает пользователь, не planner). Полная 10/90-раскладка, `DepartmentNavigationRail`,
  `DepartmentScene`/`BeforeAfterSequence`/`OutcomePanel`, диагностика, mobile touch flow,
  GSAP-анимации — вне scope (Step 5/6/7/8 по уже утверждённому 8-шаговому скелету).
- In scope (не зависит от открытых вопросов):
  ...
  - **Расположение состояния** — по `docs/09` "Возможная структура", `src/features/office-machine/`
    ...
    от **OQ-1**.
  - **Keyboard/Escape/focus** (источник — `docs/11-accessibility.md` "Keyboard": "Tab по отделам;
    Enter/Space открывает; Escape закрывает; после закрытия focus возвращается; скрытые элементы не
    фокусируются"):
    - `Escape` при активном отделе закрывает его (`CLOSE_DEPARTMENT`), фокус возвращается на
      исходный hotspot.
    - Скрытые до `ACTIVATE_CTA` хотспоты не должны быть в Tab-последовательности, пока скрыты
      (не полагаться на визуальное `opacity:0`, если элемент при этом остаётся focusable).
    - `Enter`/`Space` на видимом hotspot теперь **обязан** открывать отдел — прямая инверсия
      текущего теста Step 3 `"Enter/Space on a hotspot is a no-op"`, который заменяется, а не
      оставляется описывать устаревшее поведение.
  - **`prefers-reduced-motion` — сохранение функции, не только длительности.** Step 3 уже обнулил
    CSS transition duration глобально. Step 4 добавляет JS-управляемые переходы состояния — по
    CLAUDE.md Motion rules ("critical content must not depend on animation completion") раскрытие/
    открытие должно быть функционально завершено сразу после клика (элемент реально в DOM,
    focusable, кликабелен), независимо от визуальной анимации.
  - **URL как минимум для чтения `ROUTE_CHANGE` при boot** — при валидном `?department=<id>` в query
    string при первой загрузке не показывать голый `hero`-без-раскрытия ("шаринг ссылки не должен
    требовать лишнего клика" из `docs/05`); точная глубина (открывается ли сам отдел, а не только
    `overview`) — **OQ-3**.
  - **Регрессионное обновление существующих Step 3 тестов**, чьи текущие assertions описывают
    поведение, которое Step 4 намеренно меняет:
    - `src/tests/e2e/office-overview-keyboard.spec.ts`: `"Enter/Space on a hotspot is a no-op"` —
      заменяется; Tab-порядок и reduced-motion-тест — должны сначала выполнить `ACTIVATE_CTA`
      (клик/Enter на CTA), прежде чем измерять хотспоты.
    - `src/tests/e2e/office-overview.spec.ts`: `"renders the hero and all 5 department hotspots"` —
      ...
      `"ignores the ?department= query string..."` — переписывается целиком под поведение,
      решённое по **OQ-3** (не просто удаляется без замены); low-height (1280×500) тест — тоже
      сначала выполняет `ACTIVATE_CTA`.
    - `src/tests/unit/components/homepage/hero-copy.test.tsx`,
      `src/tests/unit/components/office/department-hotspot.test.tsx`,
      `src/tests/unit/components/office/office-semantic-map.test.tsx` — обновляются под новые
      props/обработчики (форма зависит от **OQ-1**/**OQ-2**, но сама правка — не открытый вопрос).
  - Новые unit-тесты для state-логики (`src/tests/unit/features/office-machine/*`, независимо от
    итоговой библиотеки): начальное состояние — `hero` (отделы скрыты); `ACTIVATE_CTA` →
    `overview`; `SELECT_DEPARTMENT(id)` → активен ровно один отдел; `CLOSE_DEPARTMENT`/`ESCAPE` →
    назад к `overview`; невалидный `id` в `SELECT_DEPARTMENT`/URL не создаёт "битого" состояния.
  - Новые/обновлённые e2e-тесты: скрытие хотспотов до клика при включённом JS, реальное раскрытие по
    ...
    при первой загрузке с включённым JS (см. Risks), Escape/focus-return, URL-обновление при выборе
    отдела без полной перезагрузки страницы.
- Out of scope:
...
- Dependencies: Step 3 (`COMPLETED`). Дополнительно: решение пользователя по OQ-1/OQ-2/OQ-3
  обязательно до перехода этого шага в `APPROVED` — без него Expected files/Acceptance criteria
  ниже не могут быть зафиксированы окончательно (см. пометки `[OQ-N]`).
- Expected files _(часть — условно, см. пометки; часть — точно, см. In scope выше)_:
  ...
  - `src/components/office/OfficeExperience.tsx`, `OfficeSemanticMap.tsx`, `DepartmentHotspot.tsx`
    (+ `.module.css`) — правка обязательна как минимум для скрытия-по-умолчанию/раскрытия и
    `onClick`; глубина правки (просто `onClick` vs полноценная active-content ветка) — **OQ-2**.
  ...
  - Новый(е) e2e-файл(ы), например `src/tests/e2e/office-department-selection.spec.ts`.
  ...
  - `README.md`, `WORKPLAN.md`, `WORKLOG.md`, `DECISIONS.md` (процессные).
- Acceptance criteria _(пункты финализируются после ответа пользователя на OQ-1/OQ-2/OQ-3)_:
  ...
  4. `Escape` при открытом отделе закрывает его и возвращает focus на исходный hotspot.
  5. `Enter`/`Space` на видимом hotspot открывает отдел (не no-op, в отличие от Step 3).
  6. Скрытые (нераскрытые) элементы не встречаются в Tab-последовательности.
  7. В любой момент времени активен не более одного отдела (инвариант `docs/05`), проверено
     unit-тестом на state-модуле.
  8. `?department=<валидный id>` при первой загрузке приводит к поведению, определённому **OQ-3**.
  9. `?department=<невалидный id>` не приводит к ошибке/пустой странице — деградирует к `overview`.
  10. Выбор отдела обновляет URL; переключение/закрытие не вызывает полную перезагрузку страницы
      (проверяется отсутствием полного document-навигационного события в e2e).
  11. `prefers-reduced-motion: reduce` — весь функционал (раскрытие, выбор, закрытие) работает
      идентично по результату, не только по длительности перехода.
  12. Ни один client-компонент не импортирует `src/content/*` напрямую (grep-проверка, по
      прецеденту Step 3 "grep на `'use client'`").
  13. Существующие Step 3 тесты, чьи ожидания намеренно инвертированы Step 4, реально заменены
      новыми, корректными ожиданиями — не удалены без замены и не оставлены описывать устаревшее
      поведение.
  14. Low-height fallback (`docs/08`, `@media (max-height: 700px)`, 1280×500) продолжает работать
      после раскрытия отдела: заголовок/CTA не обрезаны, документ не скроллится целиком.
  15. `npm run format:check`, `npm run lint`, `npm run typecheck`, `npm run test`, `npm run build`,
      `npm run test:e2e` — все exit 0.
  16. `git diff --stat` относительно коммита закрытия Step 3 ограничен списком Expected files.
  17. `DECISIONS.md` содержит записи по каждому решению из OQ-1/OQ-2/OQ-3 (и, если применимо, по
...
- Manual checks:
  ...
  - Клавиатура: Tab до раскрытия не достигает hotspot'ов; после раскрытия — достигает всех 5 в том
    же порядке, что в Step 3; Enter/Space открывает отдел; Escape закрывает и возвращает фокус.
  - Ручная проверка URL: клик по отделу обновляет адресную строку; кнопка "назад" браузера
    возвращает к overview/предыдущему отделу без полной перезагрузки (проверить по Network-панели
    DevTools — нет полного document-запроса).
  ...
  - Проверить 1280×500 (low-height) после раскрытия отдела — заголовок/CTA не обрезаны.
...
- Risks:
  - **Server/client рефакторинг существующих Step 3 компонентов** — первый в проекте случай, когда
    ...
    смягчается grep-проверкой (acceptance criterion 12) и `frontend-architect` review на milestone.
  ...
  - **Механизм URL-обновления в Next.js App Router не тривиален**: `router.push('/?department=…')`
    из `next/navigation` по умолчанию может вызвать серверный re-render `page.tsx` при каждой смене
    отдела — потенциально конфликтует с духом инварианта `docs/05` "не происходит полной
    перезагрузки". Альтернатива — прямой `history.pushState`/`replaceState` в обход
    `next/navigation` ("shallow routing"). Выбор — задача исполнения, но должен быть явно проверен
    acceptance criterion 10, а не просто заявлен.
  ...
  - **Момент введения Zustand/GSAP** — `docs/09` "Стек" перечисляет оба без пометки "или", формально
    выглядя как уже решённые технологии, но ни то, ни другое не установлено в `package.json`, и
    Step 1 явно вывел оба "Out of scope" (отложены на момент реальной клиентской интерактивности —
    то есть именно сейчас). Трактовка "раз в стеке — значит можно добавлять без вопроса" отклонена
    как повторение прецедента BLOCKED (Step 2, zod) — вопрос вынесен пользователю (**OQ-1**) явно.
...
**OQ-2. Граница Step 4 / Step 5 по глубине состояний выбора отдела.**
...
- (b) Step 4 реализует ТОЛЬКО `hero → overview` (`ACTIVATE_CTA`) плюс упрощённый бинарный
  ...
  какого-либо видимого контента отдела) — Step 5 берёт на себя одновременно и 10/90-раскладку, и
  полноценные `opening/switching/closing`-переходы.
