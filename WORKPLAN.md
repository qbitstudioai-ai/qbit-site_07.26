...
- Objective: Реализовать первую клиентскую интерактивность главной страницы. **Часть (1) — решена,
  ...
  `?department=<id>`. **OQ-2 теперь включает три варианта, а не два** (см. "Open questions" ниже):
  (a) Step 4 реализует полный набор `opening/active/switching/closing` с минимальным UI; (b) Step 4
  реализует только упрощённый бинарный select/close, без под-состояний; **(c) Step 4 НЕ реализует
  выбор отдела вовсе** — Step 4 ограничивается `hero ↔ overview`, а весь `SELECT_DEPARTMENT`/
  `CLOSE_DEPARTMENT`/URL-sync целиком переносится в Step 5 (это буквально соответствует уже
  утверждённой, не тронутой этим планом формулировке Step 5: "Реализовать выбор, панель отделов и
  основное поле"). Ни один из трёх вариантов не выбран planner'ом заранее — решает пользователь.
  ...
  - **Регрессионное обновление — условная часть (зависит от OQ-2/OQ-3):**
    ...
    - `src/tests/unit/components/homepage/hero-copy.test.tsx`,
      ...
      **OQ-1**/**OQ-2**; сама правка под hero/overview-видимость (`isRevealed`-подобный проп) —
      безусловна.
  - **Новые unit-тесты для state-логики — только если OQ-2 ≠ (c)**
    (`src/tests/unit/features/office-machine/*`, независимо от итоговой библиотеки): начальное
    состояние — `hero` (отделы скрыты); `ACTIVATE_CTA` → `overview`; `SELECT_DEPARTMENT(id)` →
    активен ровно один отдел; `CLOSE_DEPARTMENT`/`ESCAPE` → назад к `overview`; невалидный `id` в
    `SELECT_DEPARTMENT`/URL не создаёт "битого" состояния. **Если OQ-2 = (c)**, `office-machine`
    покрывает только переход `hero` ↔ `overview` (единственное состояние в этом шаге).
...
- **(c) Step 4 НЕ реализует выбор отдела вовсе** — ограничивается только `hero ↔ overview`
  (раскрытие пяти скрытых хотспотов по `ACTIVATE_CTA`); хотспоты после раскрытия остаются no-op при
  клике/Enter/Space, как в Step 3, до тех пор, пока Step 5 не реализует `SELECT_DEPARTMENT`/
  `CLOSE_DEPARTMENT`/URL-sync/`opening`/`active`/`switching`/`closing` с нуля вместе с 10/90-
  раскладкой. Наиболее буквально соответствует уже утверждённой формулировке Step 5. Минус:
  Step 5 получает больший объём (state machine + раскладка одновременно), в отличие от (a), где
  Step 5 — только визуальный слой поверх уже работающей логики.
