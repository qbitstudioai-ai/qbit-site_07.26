...
- **Manual checks — НЕ выполнены.** План помечает их «обязательно»: desktop-проход
  открыть/переключить/закрыть, DevTools reduced-motion, DevTools Performance (риск джанка на
  transform-зуме крупного фотослоя назван в Risks самого шага). Автоматические проверки их не
  заменяют; отсутствие тяжёлого loop подтверждено косвенно (`document.getAnimations()` = 0 в покое,
  непрерывных анимаций в коде нет), замера джанка нет. Остаётся за пользователем.
