...
- Status: `PROPOSED`
- Objective: Адаптировать уже работающую офис-цепочку `hero → overview → выбор отдела`
  (state machine, URL-sync, focus management — `src/features/office-machine/reducer.ts`,
  `OfficeMachine.tsx`, `url-sync.ts`, реализованы и приняты в Step 5/Step 6, `COMPLETED`) под
  ширину viewport ≤767px (`docs/08-responsive-behavior.md` "Mobile ≤767") для pointer/touch-ввода,
  по Core concept CLAUDE.md ("Mobile is not a scaled-down desktop office") и разделам "Mobile"
  `docs/03-office-map.md`/`docs/08-responsive-behavior.md`/`docs/14-acceptance-criteria.md`: вместо
  пространственной "карты офиса" (абсолютно позиционированные хотспоты) и буквальной 10/90-сетки —
  линейный touch-первый путь (`docs/08`: логотип → hero → обзор офиса → список/карусель отделов →
  активная карточка → до/после → CTA, с явной кнопкой назад). **Этот шаг не переделывает и не
  расширяет state machine** — используются те же состояния (`hero`/`overview`/
  `department-opening`/`department-active`/`department-switching`/`department-closing`) и те же
  действия (`ACTIVATE_CTA`/`SELECT_DEPARTMENT`/`SWITCH_DEPARTMENT`/`CLOSE_DEPARTMENT`/`ESCAPE`) без
  единого нового состояния/действия в редьюсере — адаптируются только компоненты, рендерящие каждое
  состояние, и (в зависимости от ответа на OQ-M3 ниже) то, какие именно действия мобильный UI вообще
  диспетчерит.

  Точное распределение ответственности этого шага зависит от четырёх реальных, не решённых ни одним
  approved-документом развилок — они вынесены как **OQ-M1–OQ-M4** (см. отдельный раздел "Open
  questions" ниже) и явно помечены в тексте этого черновика инлайн, а не решены молча. План ниже
  описывает связную рабочую конструкцию **по умолчанию** (нужную, чтобы acceptance criteria были
  конкретными и проверяемыми уже сейчас), но каждое место, зависящее от OQ, помечено — и не может
  считаться `APPROVED` до ответа пользователя на все четыре вопроса (тот же процесс, что уже был
  пройден для OQ-A/B/C/D при планировании Step 5/Step 6).

  По умолчанию (пока не решено иначе пользователем): breakpoint — строго `≤767px` по ширине
  (`docs/08`, буквальное значение уже зафиксировано документом, не открытый вопрос); Tablet
  768–1279px — явный, названный gap вне scope этого шага (**OQ-M1**); переключение отделов внутри
  overview — вертикальный прокручиваемый список (не карусель), полноэкранная карточка отдела без
  одновременно видимого rail, возврат к другому отделу — только через список (**OQ-M3**); свайп —
  не реализуется в этом шаге, навигация только тап/клавиатура (**OQ-M2**); `interactionHint` не
  рендерится на ≤767px (**OQ-M4**).

- In scope:
  - **Механизм breakpoint — только CSS, без JS/`matchMedia`.** Media query `max-width: 767px` в
    существующих `*.module.css`-файлах, по прямой аналогии с уже принятым паттерном
    `@media (max-height: 700px)` ("Низкий desktop", Steps 3–6). Один и тот же DOM/компонентное
    дерево рендерится на всех ширинах; переключение раскладки — только CSS. Причина явного отказа
    от JS-детекции ширины (`window.matchMedia`/resize-listener): такой подход воспроизвёл бы
    именно тот класс риска гидратации/рассинхронизации сервер-клиент, который уже один раз был
    пропущен в Step 4 (реальный, найденный пользователем hydration-mismatch баг) и явно назван
    процессным риском в Steps 5/6 — CSS-only подход этот риск структурно исключает. Технический
    выбор исполнителя, низкий риск, не требующий отдельного вопроса (прецедент: Step 3
    "сортировка хотспотов").
  - **Рефакторинг позиционирования `DepartmentHotspot.tsx`: inline `left/top/width/height` →
    CSS custom properties.** Сейчас позиция хотспота задаётся напрямую через inline `style={{left,
    top, width, height}}` (проценты из `office-zones.json`) — inline-стиль имеет более высокую
    специфичность, чем любое внешнее правило класса, поэтому `@media (max-width: 767px)`-правило в
    `DepartmentHotspot.module.css` не сможет чисто переопределить `position`/раскладку без
    `!important` (хрупкое решение). Вместо этого `style` передаёт значения через CSS custom
    properties (`style={{ "--zone-x": \`${zone.x}%\`, ... }}`), а `.hotspot { left: var(--zone-x);
    ... }` в CSS Module — тогда более поздний по каскаду `@media` c тем же классом-селектором
    переопределяет `position`/`left`/`top`/`width`/`height` без `!important` штатным механизмом
    каскада. Контракт компонента (`zone`/`department`/`onSelect` props, accessible name,
    `aria-describedby`) не меняется — реальный desktop-рендер должен остаться пиксель-в-пиксель
    идентичным (регрессия существующих Step 3 e2e-измерений геометрии). Технический выбор
    исполнителя, низкий риск (прецедент: Step 3 "сортировка хотспотов"), явно раскрыт здесь, а не
    скрыт как побочный эффект.
  - **Overview на ≤767px: вертикальный список отделов вместо пространственной карты**
    (`OfficeSemanticMap.tsx`/`.module.css`, `DepartmentHotspot.module.css`) — тот же `<nav
    aria-label="Отделы компании">`/`<ul id="office-map">`/`DepartmentHotspot`, без нового
    компонента: на ≤767px `.hotspot` теряет `position: absolute` (через CSS-var-рефакторинг выше),
    становится `position: static`, `width: 100%`; `.zoneList` в текущем коде не имеет `display:
    flex` вовсе (`<ul>` со стандартным block-рендерингом) — на ≤767px явно добавляется `display:
    flex; flex-direction: column`, `overflow-y` вместо `overflow: hidden`, без фиксированного
    `min-height: 340px` (актуален только для %-абсолютной раскладки desktop). Каждая запись —
    полноширинный тап-таргет.
    `overviewProblem` на ≤767px **не скрыт через hover/focus-gating** — постоянно видим в разметке
    (снятие CSS `opacity:0/max-height:0` из `.problem` при `≤767px`), потому что touch не имеет
    состояния hover, а полагаться на programmatic focus как единственный способ раскрытия текста на
    touch-устройстве без клавиатуры means текст никогда не будет виден — соответствует `docs/14`
    "touch не зависит от hover" и `docs/08` "Mobile copy" ("короткое описание"). Технический выбор
    исполнителя (продолжение уже принятого в Step 3 паттерна "текст в DOM всегда, видимость через
    CSS"), не новая архитектура.
  - **[OQ-M4] `interactionHint`.** По умолчанию не рендерится на ≤767px (`OfficeExperience.tsx`:
    условный `className`/CSS-скрытие `<p className={styles.hint}>`, без удаления из
    `data/homepage-copy.json` — контент не редактируется). Причина: единственная существующая
    строка ("Наведите курсор на отдел") буквально описывает hover, которого на touch нет; список
    отделов на ≤767px самообъясним визуально (полноширинные тап-элементы), замены текстом не
    требует. Помечено как открытый вопрос, а не решено молча — см. OQ-M4 ниже, возможны другие
    варианты.
  - **Активный отдел на ≤767px: (почти) полноэкранная карточка вместо 10/90.**
    `OfficeExperience.module.css`: `.shell10x90` при `≤767px` — `grid-template-columns: 1fr`,
    `grid-template-areas: "main"` (без `"rail"`). **Важно (исправлено после skeptic Phase A round
    1, находка Major):** сама по себе смена `grid-template-areas` не скрывает `.railArea` — когда
    `grid-area: rail` не совпадает ни с одной именованной областью, элемент не исчезает, а
    возвращается к auto-placement и всё равно рендерится (в один столбец, ниже `.mainArea`).
    Требуется явное `.railArea { display: none; }` внутри того же `@media (max-width: 767px)` —
    только это реально убирает `DepartmentNavigationRail` из видимого дерева и Tab-порядка на
    мобильном breakpoint. С этой явной правкой — прямое следствие `docs/03` Mobile ("не уменьшенная
    карта, а отдельный touch-путь... полноэкранная карточка отдела") и `docs/08` ("Не использовать
    буквальный 10/90"). [OQ-M3] Это поведение (список → полноэкранная карточка → только "назад" к
    списку, без прямого переключения между уже открытыми отделами) — вариант (a) OQ-M3 ниже; другие
    варианты этого вопроса меняют, показывается ли какой-то способ смены отдела
    из активной карточки.
  - **Явная кнопка "назад".** Переиспользуется уже существующая кнопка «Закрыть»
    (`DepartmentExperience.tsx`, `onClose` → `CLOSE_DEPARTMENT`) — функционально идентична
    требованию `docs/08` "Есть явная кнопка назад"/`docs/03` Mobile; новая копия/label не вводится
    (нет отдельного поля в `data/homepage-copy.json`/`data/departments.json` под "назад" отдельно
    от "закрыть" — технический выбор, не продуктовое решение, см. Risks на семантический нюанс
    "закрыть" vs "назад").
  - **`DepartmentCopy`/`OutcomePanel`/`DepartmentCTA`/`DepartmentExperience` — переиспользуются без
    изменения контракта**, только `.module.css`-проходка на ≤767px (typography/spacing scale,
    аналогично уже существующему `@media (max-height: 700px)` в этих же файлах): полноширинная
    CTA-кнопка и кнопка «Закрыть», достаточные тап-таргеты (см. acceptance criteria), без
    горизонтального overflow.
  - **Hero (`HeroCopy.tsx`/`.module.css`) на ≤767px** — `.ctaRow` переходит в колонку
    (полноширинные кнопки), типографика уменьшается по тому же паттерну, что уже есть для
    `max-height: 700px`; `valuePoints` продолжают переноситься (`flex-wrap` уже есть).
  - **Header (`Header.module.css`) на ≤767px** — точечная проверка/правка, чтобы лого + текстовый
    бренд не перекрывались и не обрезались на узких экранах (320–375px).
  - **Проверка `<meta name="viewport">`.** Next.js App Router по умолчанию должен добавлять
    `width=device-width, initial-scale=1` без явного `viewport`-экспорта — это подтверждается
    однократной ручной/e2e-проверкой рендеринга `<head>`, а не считается фактом заранее. Если тег
    фактически отсутствует — `src/app/layout.tsx` получает явный `export const viewport: Viewport =
    {...}` (условный Expected file, только при подтверждённом отсутствии).
  - **Тап-таргеты ≥44×44 CSS px** для всех интерактивных элементов, затронутых/добавленных на
    ≤767px (hero CTA, элементы списка отделов, CTA отдела, кнопка «Закрыть») — тот же числовой
    прецедент, что уже использован в Step 3 (low-height fallback, измеренные 81.6–122.4px против
    исходных 24.7px).
  - **Клавиатура на touch-устройстве с подключённой клавиатурой не теряется** — Tab достигает
    элементов списка, Enter/Space открывает отдел, Tab внутри отдела достигает CTA/«Закрыть»,
    Escape закрывает и возвращает focus — регрессия уже реализованного в Step 5/6 поведения,
    перепроверенная именно на ≤767px раскладке (не полагаться на то, что оно "просто продолжит
    работать", раз DOM/фокус-менеджмент не менялись).
  - **`prefers-reduced-motion` на ≤767px** — переиспользуется существующее глобальное CSS-правило
    (`globals.css`) и уже существующие CSS `@keyframes`/`animation`-классы
    (`DepartmentExperience.module.css` `.opening`/`.switching`/`.closing`) — новая моторная система
    не вводится; если добавляется мобильно-специфичное визуальное усиление перехода (например,
    сдвиг вместо/вместе с fade для "полноэкранной карточки"), оно обязано использовать те же классы
    и ту же длительность, не создавая параллельный механизм анимации.
  - **Инвариант "без скролла документа" переносится на ≤767px** — `.shell {overflow: hidden}`
    (`HomepageShell.module.css`) не меняется; избыток контента (список отделов, длинная карточка
    отдела) обрабатывается уже существующим внутренним скроллом (`.office`/`.experience` —
    `overflow-y: auto`, без изменений) — регрессия и новая явная проверка именно на узких И
    одновременно низких viewport (комбинация `max-width` и `max-height` media queries, не
    тестированная ранее ни разу).
  - **[OQ-M2] Свайп.** По умолчанию не реализуется в этом шаге (`docs/08`: "Свайп только как
    дополнительный способ" — не обязателен); вся навигация — тап/клавиатура. Если ответ на OQ-M2
    выбирает реализацию свайпа сейчас — самописный обработчик `touchstart`/`touchmove`/`touchend`
    без новой npm-зависимости, строго дополнительный (не единственный) способ, с явным порогом
    расстояния/направления, не мешающий вертикальному скроллу панели.
  - **Server/client граница** (не новое решение) — `src/content/*.ts` остаются server-only; для
    этого шага не требуется ни одного нового `'use client'`-компонента сверх уже существующего
    `OfficeMachine.tsx` (все изменения — CSS + переиспользуемые компоненты + point-in-time
    CSS-var-рефакторинг `DepartmentHotspot`).
  - **Обновлённые unit-тесты:** `department-hotspot.test.tsx` (регрессия после CSS-var-рефакторинга
    — те же значения `left/top/width/height`, теперь через custom properties, а не напрямую);
    `office-semantic-map.test.tsx`/`office-experience.test.tsx` (условный рендер `interactionHint`
    и/или rail в зависимости от финального ответа на OQ-M3/OQ-M4).
  - **Новые e2e-тесты:** `src/tests/e2e/mobile-touch-flow.spec.ts` — `test.use({ viewport: {width:
    375, height: 812}, hasTouch: true, isMobile: true })` (конкретный механизм — inline `test.use`
    в новом spec-файле в рамках существующего `chromium`-проекта, либо отдельный Playwright project
    в `playwright.config.ts` — решается при исполнении, по прецеденту "механизм dev-mode e2e
    решается при исполнении", Steps 5/6): полный поток (hero → `ACTIVATE_CTA` тапом → список
    отделов → тап по отделу → карточка отдела видна → CTA/«Закрыть» тапаются
    (`page.tap()`/`locator.tap()`) → возврат к списку); прямой `?department=<id>` на мобильном
    viewport; измерение тап-таргетов (`boundingClientRect` ≥44×44); отсутствие горизонтального
    скролла; отсутствие постраничного вертикального скролла (регрессия инварианта на новой
    комбинации ширины/высоты); `prefers-reduced-motion` на ≤767px; клавиатурная регрессия на том же
    viewport; отсутствие console/hydration-mismatch ошибок — отдельно в `npm run dev` и отдельно в
    production-сборке (тот же обязательный dual-mode паттерн, что в Steps 5/6); полная регрессия
    существующих desktop (≥1280px) e2e-сьютов без ослабления ожиданий (CSS для ≤767px не должен
    протечь на бóльшие ширины).
- Out of scope:
  - **[OQ-M1] Tablet 768–1279px** — по умолчанию явный, названный gap вне scope этого шага (см.
    OQ-M1); не реализуется здесь, если пользователь не выберет иное до утверждения плана.
  - **`hover`/`pointer` CSS media-feature-based адаптация** (`docs/08` "Учитывать": "hover
    capability; fine/coarse pointer") — например, тачскрин-ноутбук на desktop-ширине или планшет с
    мышью — не реализуется; breakpoint только по ширине viewport (`max-width: 767px`), не по
    capability-детекции устройства. Явно поднятый, не молчаливый gap — см. Risks.
  - Новые npm-зависимости (carousel/gesture-библиотеки) — не вводятся, если ответ на OQ-M2 их не
    потребует явно (тогда — отдельная запись `DECISIONS.md` по прецеденту `zod`, Step 2).
  - GSAP/WebGL/Canvas — по-прежнему не вводятся (существующий инвариант, Step 3/5/6 Risks/Out of
    scope).
  - Правка `data/departments.json`/`data/homepage-copy.json`/`data/office-zones.json` — контент не
    редактируется этим шагом (если ответ на OQ-M4 не выберет вариант (c), требующий новой строки
    копии — тогда потребуется отдельное согласование, аналогично `DECISIONS.md`-прецедентам).
  - Новые состояния/действия редьюсера `office-machine` — не вводятся; state machine используется
    как есть.
  - Диагностика (`OPEN_DIAGNOSTIC`/`diagnostic-*`), `contact-open`, `OfficeVisualLayer`, WebGL/
    Canvas, `FallbackExperience`/полноценный `error-fallback`, автоматизированное axe-сканирование,
    реальная навигация на `/solutions/*`, поддержка браузерных «назад»/«вперёд» (`popstate`),
    дозаполнение `data/departments.json` (`beforeSteps`/`automationSteps`/`visual`), правка
    `MANIFEST.json`, CI pipeline — все уже установленные для Steps 3–6 исключения остаются в силе
    без изменений.
  - `BeforeAfterSequence`/структурированное "до/после" (шаг 6 мобильного пути `docs/08`) — по-
    прежнему не создаётся (решение OQ-C, Step 6, `DECISIONS.md` 2026-07-15) — мобильная карточка
    отдела показывает тот же неструктурированный `problem`/`outcomes[]`, что и desktop, не более.
  - Реальное устройство/лаборатория тестирования (VoiceOver/TalkBack на реальном телефоне) —
    `docs/11` "Тесты" перечисляет их как желаемые проверки, но инфраструктуры для них в проекте нет
    ни на одном из предыдущих шагов (тот же класс "аспирационной", не блокирующей проверки, что
    ручной axe DevTools в Step 3); остаётся ручной DevTools device-toolbar эмуляцией + Playwright
    Chromium touch-эмуляцией, не полноценным cross-device/cross-engine покрытием.
  - Landscape-ориентация как отдельный, специфицированный критерий приёмки — не вводится отдельным
    acceptance criterion сверх уже существующего low-height паттерна; проверяется только в рамках
    Risks/Manual checks, не как формальное требование.
- Dependencies: Step 6 (`COMPLETED`). **Найдено при skeptic Phase A round 1 (Critical): Step 6 на
  момент этого планирования только застейджен (`git add -A`), не закоммичен** —
  `git diff --cached --stat` показывает 24 файла, `WORKLOG.md` Entry 6 сама это явно фиксирует
  ("Step 6 ещё не закоммичен"). Реальный "коммит закрытия Step 6", на который ссылаются acceptance
  criterion 18 и Manual checks ниже, пока не существует — до его появления эти пункты не
  исполнимы буквально. **Дополнительная зависимость этого шага:** коммит, фиксирующий закрытие
  Step 6 (с явного разрешения пользователя — Safety-правило CLAUDE.md "Never commit... unless
  requested"/"Do not create commits unless requested"), должен появиться до начала реализации
  Step 7 — иначе изменения Step 6 (всё ещё в индексе) и Step 7 окажутся неразделимы в одном первом
  коммите, что нарушает потрейсность/rollback-инвариант, соблюдавшийся во всех Steps 1–5. Ответы
  пользователя на OQ-M1/OQ-M2/OQ-M3/OQ-M4 (см. "Open questions" ниже) должны быть получены и
  зафиксированы в `DECISIONS.md` до перевода этого плана в `APPROVED` и до начала реализации.
- Expected files:
  - `src/components/office/DepartmentHotspot.tsx` (+ `.module.css`) — CSS-var-рефакторинг
    позиционирования, `≤767px`-раскладка списка, снятие hover-gating у `.problem`.
  - `src/components/office/OfficeSemanticMap.tsx` (+ `.module.css`) — `≤767px` вертикальный список
    (`flex-direction: column`, без фиксированного `min-height`/`overflow: hidden` карты).
  - `src/components/office/OfficeExperience.tsx` (+ `.module.css`) — `≤767px`-override
    `.shell10x90` (одна колонка, без `"rail"`), условный рендер `interactionHint`/rail в
    зависимости от OQ-M3/OQ-M4.
  - `src/components/office/DepartmentNavigationRail.tsx` (+ `.module.css`) — `≤767px`: не
    рендерится (или рендерится иначе — зависит от OQ-M3).
  - `src/components/departments/DepartmentExperience.tsx` (+ `.module.css`) — `≤767px`-типографика/
    spacing, полноширинные CTA/«Закрыть»; возможный swipe-обработчик (OQ-M2).
  - `src/components/departments/DepartmentCopy.module.css`, `OutcomePanel.module.css`,
    `DepartmentCTA.module.css` — `≤767px`-проходка typography/touch-target.
  - `src/components/homepage/HeroCopy.tsx` (+ `.module.css`) — `≤767px`-раскладка CTA/типографика.
  - `src/components/homepage/Header.module.css` — `≤767px`-проходка (при необходимости).
  - `src/app/layout.tsx` — условно, только при подтверждённом отсутствии дефолтного `viewport`
    meta тега.
  - `playwright.config.ts` — условно, только если выбран отдельный Playwright project вместо
    inline `test.use()` (решается при исполнении).
  - Новый `src/tests/e2e/mobile-touch-flow.spec.ts` (возможно, второй файл для клавиатурной
    регрессии — по аналогии с раздельными `office-overview.spec.ts`/
    `office-overview-keyboard.spec.ts` Step 3).
  - Обновления существующих unit-тестов, перечисленных в In scope.
  - `README.md`, `WORKPLAN.md`, `WORKLOG.md`, `DECISIONS.md` (процессные, включая записи по
    OQ-M1–OQ-M4).
- Acceptance criteria:
  1. На viewport шириной ≤767px состояние `overview` рендерит отделы вертикальным списком
     полноширинных тап-элементов, а не пространственной абсолютно-позиционированной картой —
     проверено структурно (нет наложения/абсолютного позиционирования) и геометрически
     (`boundingClientRect`).
  2. `overviewProblem` виден в разметке для каждой записи списка на ≤767px без наведения/focus —
     touch не зависит от hover (`docs/14`).
  3. Тап (`page.tap()`) по записи списка открывает соответствующий отдел: содержимое занимает
     большую часть viewport (не буквальный 10/90), без полной перезагрузки/навигации документа.
  4. Явная, всегда видимая кнопка «назад» (переиспользованная «Закрыть») присутствует и тапается,
     пока отдел активен на ≤767px; активация возвращает к списку, эффект идентичен `Escape`.
  5. `DepartmentCTA` реально достижима без блуждания — не обрезана, находится в пределах viewport
     или доступна через тот же внутренний скролл-паттерн, что и остальной проект; измерено минимум
     на двух характерных высотах (например, 812px и ~650px).
  6. Все интерактивные тап-таргеты, затронутые/добавленные для ≤767px (hero CTA, записи списка,
     CTA отдела, кнопка «Закрыть»), измеряются ≥44×44 CSS px.
  7. Нет горизонтального скролла ни на одной проверенной ширине ≤767px
     (`document.documentElement.scrollWidth <= window.innerWidth`).
  8. Документ по-прежнему не скроллится целиком на ≤767px (`.shell` инвариант) — переполнение
     обрабатывается только внутренним скроллом соответствующей панели, регрессия проверена именно
     на мобильной ширине, а не унаследована по предположению.
  9. Прямой URL `?department=<id>` на viewport ≤767px открывает нужный отдел сразу при первой
     загрузке — идентично уже реализованному desktop-поведению (Step 5), теперь проверено и на
     мобильной ширине.
  10. `prefers-reduced-motion: reduce` на ≤767px: переходы мгновенные/короткий fade, все
      функциональные критерии выше остаются истинными.
  11. Клавиатура не теряется на ≤767px: Tab достигает записей списка, Enter/Space открывает,
      Tab внутри отдела достигает CTA/«Закрыть», Escape закрывает и возвращает focus на исходную
      запись списка.
  12. **[OQ-M2-зависимый]** Если свайп реализован — горизонтальный свайп работает как
      дополнительный, не единственный способ навигации (тап/клавиатура работают идентично без
      свайпа). Если свайп не реализован (значение по умолчанию) — критерий переформулирован как
      "навигация на ≤767px на 100% доступна через тап/клавиатуру, свайп в этом шаге отсутствует",
      и это явно зафиксировано в Out of scope, а не обнаружено постфактум.
  13. Ни один client-компонент, введённый/изменённый этим шагом, не импортирует `src/content/*`
      напрямую (grep-регрессия, тот же инвариант, что в Steps 3–6) — ожидаемо тривиально верно, так
      как новых client-компонентов сверх `OfficeMachine.tsx` не вводится.
  14. Desktop-поведение (≥1280px) из Steps 3–6 не регрессирует — полный прогон существующих
      desktop e2e-сьютов без ослабления ожиданий; CSS для ≤767px не протекает на большие ширины.
  15. `<meta name="viewport">` присутствует в отрендеренном HTML (`width=device-width,
      initial-scale=1` или эквивалент) — проверено явно, не предположено.
  16. Нет console/page errors на всём мобильном потоке (раскрытие → список → открытие → закрытие →
      прямой URL) — отдельно в `npm run dev`, отдельно в production-сборке.
  17. `npm run format:check`, `npm run lint`, `npm run typecheck`, `npm run test`,
      `npm run build`, `npm run test:e2e` — все exit 0.
  18. `git diff --stat` относительно коммита закрытия Step 6 ограничен списком Expected files.
  19. `DECISIONS.md` содержит записи по OQ-M1/OQ-M2/OQ-M3/OQ-M4 с реальным согласованием
      пользователя, полученным до начала реализации, а не проставленные исполнителем.
  20. Ни одна новая npm-зависимость не добавлена в `package.json`/`package-lock.json`, если ответ
      на OQ-M2 явно не потребовал этого (тогда — отдельная согласованная запись `DECISIONS.md`, а
      не тихое добавление).
- Verification commands:
  ```bash
  npm run format:check
  ...
  npm run test:e2e
  ```
  Плюс обязательная dev-mode console-проверка (тот же процессный прецедент, что Steps 5/6):
  ```bash
  npm run dev
  # headless Playwright против http://localhost:3100 на мобильном viewport (375×812, hasTouch,
  # isMobile): пройти полный поток (раскрыть → список → открыть отдел → закрыть → прямой
  # ?department=<id>) и зафиксировать console/pageerror — ожидается [].
  # Остановить dev-сервер после проверки.
  ```
  Если выбран отдельный Playwright project для мобильной эмуляции: дополнительно
  `npx playwright test --project=<имя>` в рамках того же `npm run test:e2e`.
- Manual checks:
  - Chrome DevTools device toolbar: минимум три пресета — iPhone SE (375×667), типичный Android
    (например, 393×851), и узкий крайний случай 320px — подтвердить отсутствие горизонтального
    скролла, необрезанных CTA, читаемого списка/карточки.
  - Touch-симуляция в DevTools (или headed Playwright) — пройти полный поток тапами, не кликами
    мыши.
  - `prefers-reduced-motion: reduce` на мобильном viewport.
  - Быстрый регрессионный проход на desktop ≥1280px — подтвердить отсутствие визуальных изменений.
  - `git diff --stat` относительно коммита закрытия Step 6 — подтвердить рамки Expected files.
  - Реальное устройство — опционально, не блокирует приёмку шага при отсутствии (тот же
    аспирационный статус, что ручной axe DevTools в Step 3); при пропуске — зафиксировать как
    known limitation, не молчать.
- Risks:
  - **Специфичность inline-стилей `DepartmentHotspot`** — устраняется CSS-var-рефакторингом (см.
    In scope), но сам рефакторинг — правка уже принятого в Step 3 механизма позиционирования;
    требует точной регрессии существующих desktop e2e-измерений геометрии, не только новых
    мобильных тестов.
  - **`interactionHint` буквально предписывает hover** — реальный контент-конфликт с Motion rules
    CLAUDE.md, не решается молча (OQ-M4).
  - **Tablet 768–1279px остаётся необработанным** при выборе OQ-M1 = (a)/(c) — реальный,
    зафиксированный gap, не скрытый; при (c) требует отдельного Amendment к утверждённому
    9-шаговому скелету, аналогично Amendment 3.
  - **`hover`/`pointer`-capability edge cases** (тачскрин-ноутбук на desktop-ширине, планшет с
    мышью) — явно вне scope; breakpoint только по ширине, не по capability.
  - **Переиспользование «Закрыть» для роли «назад»** — минимальный семантический дрейф (не
    буквально то же действие в UX-терминах, хотя функционально идентично); не блокирует, но
    отмечено, чтобы не выглядеть недосмотром при демонстрации.
  - **`100dvh`/`overflow:hidden`-инвариант на реальных мобильных браузерах** — динамическое
    показывание/скрытие адресной строки в реальных мобильных браузерах — известный источник
    расхождений высоты viewport, которые Playwright-эмуляция воспроизводит не полностью; остаточный
    риск без реальной device-фермы.
  - **Playwright touch-эмуляция — только Chromium**, не отражает Safari/iOS touch-специфику
    (webkit) — существующее ограничение всего e2e-набора проекта (единственный Playwright project
    `chromium`), не новое именно для этого шага, но особенно заметное на mobile.
  - **Свайп без библиотеки (при выборе OQ-M2 = (a))** — нетривиальный класс багов (различение
    горизонтального свайпа и вертикального скролла панели) — прямая причина, почему это вынесено
    как открытый вопрос, а не решено тихо.
  - **Вложенные независимо скроллящиеся контейнеры** (найдено при skeptic Phase A round 1): на
    ≤767px и `.office` (уже `overflow-y: auto`), и `.zoneList` (переходит на `overflow-y: auto` в
    этом шаге) — независимо скроллящиеся родитель/потомок. На touch-устройстве это известный
    источник "залипающего"/неочевидного скролла (жест на списке может прокручивать не тот
    контейнер, который ожидает пользователь) — требует ручной проверки на реальном touch
    (DevTools throttling insufficient), не только автоматизированной проверки отсутствия
    постраничного скролла.
  - **Content gap `docs/12` vs `data/departments.json`** — по-прежнему в силе; "до/после" (шаг 6
    мобильного пути `docs/08`) показывается тем же неструктурированным `problem`/`outcomes[]`, что
    и desktop, без новых ожиданий сверх уже принятого в Step 6 решения (OQ-C).
  - Scope creep: соблазн реализовать полную адаптивную hover/pointer-детекцию, Tablet или
    carousel-библиотеку "заодно" — явно отклоняется до отдельного решения пользователя по
    соответствующему OQ.
- Rollback: `git revert` диапазона коммитов Step 7 (аддитивно/CSS-первично относительно Step 6—
  данные и редьюсер не меняются, только компоненты/CSS/тесты). Деструктивный `git reset --hard` —
  только с явного разрешения пользователя, как в предыдущих шагах.
- Skeptic verdict (review плана, Phase A): **round 1** — `FAIL` (Critical×1, Major×1, Minor×2): (1)
  Critical — план ссылался на "коммит закрытия Step 6" (acceptance criterion 18, Manual checks) как
  на существующий, хотя Step 6 на тот момент был только застейджен (`git add -A`), не закоммичен —
  такой коммит не существовал; (2) Major — описанный механизм скрытия `DepartmentNavigationRail` на
  ≤767px (снятие `"rail"` из `grid-template-areas`) технически не скрывает элемент — CSS Grid
  возвращает его к auto-placement, он продолжает рендериться; (3) Minor — не назван риск вложенных
  независимо скроллящихся контейнеров (`.office`+`.zoneList`, оба `overflow-y: auto` на ≤767px);
  (4) Minor — `.zoneList` в реальном коде не имеет `display: flex`, план не упоминал явное
  добавление. Все 4 устранены точечно: (1) `Dependencies` дополнена честным раскрытием + явной
  дополнительной зависимостью (коммит закрытия Step 6 — с разрешения пользователя — должен появиться
  до старта реализации Step 7); (2) добавлено явное `.railArea { display: none; }` внутри
  `@media (max-width: 767px)`; (3) добавлен пункт в Risks; (4) добавлено явное упоминание в тексте.
  **round 2** — `FAIL` (Major×1, чисто bookkeeping): все 4 находки round 1 независимо
  переподтверждены устранёнными и технически корректными (сверено с реальным застейдженным кодом
  `OfficeExperience.module.css`/`OfficeSemanticMap.module.css`/`DepartmentNavigationRail.module.css`
  построчно) — но сами поля `Skeptic verdict`/`Skeptic findings` этой секции оставались
  плейсхолдером `_(ожидает review плана)_` после того, как round 1 уже реально состоялся, и ни
  `WORKPLAN.md`, ни `WORKLOG.md` не фиксировали историю review — тот же класс находки, что уже
  многократно ловился на Step 5/6 (например, Step 6 Phase B round 2). Устранено этой же правкой:
  поля заполнены реальной историей round 1/round 2, добавлена запись в `WORKLOG.md`. **round 3**
  (финальный) — `PASS`: bookkeeping-поля проверены на отсутствие дублей (по прецеденту Step 6
  round 5), `WORKLOG.md` Entry 7 сверена построчно с перечнем собственных подразделов, все 4
  находки round 1 независимо переподтверждены устранёнными и технически корректными, git-индекс
  синхронизирован, `src/**` не менялся. Цикл коррекции плана завершён.
- Skeptic findings (Phase A): round 1 — 1 Critical + 1 Major + 2 Minor (см. выше), все устранены;
  round 2 — 1 Major (bookkeeping-поля `Skeptic verdict`/`Skeptic findings` не заполнены после
  реального round 1), устранён; round 3 — Blocker/Critical/Major/Minor нет, план готов к показу
  пользователю.
- Completion evidence: _(план прошёл skeptic Phase A round 3 `PASS`; ждёт: (a) ответов пользователя
  на OQ-M1–OQ-M4; (b) решения пользователя о коммите, закрывающем Step 6 — до перевода в `APPROVED`
  и начала реализации.)_

## Open questions (Step 7) — требуют ответа пользователя до `APPROVED`
