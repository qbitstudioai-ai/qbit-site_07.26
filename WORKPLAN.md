...
- Acceptance criteria:
  ...
  3. 133 unit + 101 e2e зелёные без изменений (смена значений токенов не ломает поведение).
...
- Manual checks: Desktop/Tablet/Mobile на `http://localhost:3100` — hero, overview, department-active,
  ...
  различим; проверить оба варианта `prefers-color-scheme`.
- Risks:
...
- Acceptance criteria:
  ...
  6. 133 unit + 101 e2e (+ новые) зелёные; ни один прежний тест не ослаблен.
- Verification commands: `npm run format:check`, `npm run lint`, `npm run typecheck`, `npm run test`,
  `npm run build`, `npm run test:e2e`.
...
- In scope:
  - `departmentPhotos.ts`: карта `departmentId → сцена отдела` (адаптивная производная), рядом с уже
    существующей картой миниатюр.
  - `OfficeExperience.tsx` (ветка `department-active`): рендер сцены активного отдела через `OfficePhoto`
    вместо общего `officeBackgroundPhoto`; сцена монтируется лениво (только активный отдел, как сейчас
    фон монтируется только в этой ветке — соответствует `docs/10` «detail assets — при выборе»).
  - Настройка затемнения/десатурации (`.backgroundPhoto`, токены Step 11) и читаемости `PainGainPanel`/
    `DepartmentExperience` поверх фото — измеренный контраст ≥ WCAG AA.
...
- Out of scope:
  - Overview-сцена — Step 12; motion-переход — Step 16; кадрирование под планшет/телефон — Step 15.
  - Изменение pain/gain-контента и state machine.
- Expected files:
  ...
  - `src/components/office/OfficeExperience.tsx`, `OfficeExperience.module.css` (per-department фон,
    скрим).
  - `src/components/departments/PainGainPanel.module.css` / `DepartmentExperience.module.css` (читаемость
    поверх фото, при необходимости).
...
- Acceptance criteria:
  ...
  5. 133 unit + 101 e2e (+ новые) зелёные; прежние не ослаблены.
- Verification commands: `npm run format:check`, `npm run lint`, `npm run typecheck`, `npm run test`,
  `npm run build`, `npm run test:e2e`.
...
- Acceptance criteria:
  ...
  4. 133 unit (+ новые) и 101 e2e зелёные; прежние не ослаблены.
- Verification commands: `npm run format:check`, `npm run lint`, `npm run typecheck`, `npm run test`,
  `npm run build`, `npm run test:e2e`.
