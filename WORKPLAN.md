...
- (b) Step 4 реализует ТОЛЬКО `hero → overview` (`ACTIVATE_CTA`) плюс упрощённый бинарный
  ...
  какого-либо видимого контента отдела), но Step 5 берёт на себя одновременно и 10/90-раскладку, и
  полноценные `opening/switching/closing`-переходы — больший объём работы в одном будущем шаге, чем
  в варианте (a).
- (c) Step 4 не реализует выбор отдела вовсе — ограничивается только `hero ↔ overview` (раскрытие
  ...
  ("Реализовать выбор, панель отделов и основное поле"), но Step 5 получает ещё больший объём
  (state machine с нуля + раскладка одновременно), чем в (a) или (b).
