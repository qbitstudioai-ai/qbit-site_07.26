...
- Status: `COMPLETED` (skeptic Phase B: раунд 1 `FAIL` — две blocking-находки, обе подтверждены
  замерами и исправлены в рамках scope; раунд 2 `PASS`, все AC выполнены и подтверждены независимым
  прогоном против production-сборки. Пять non-blocking находок раунда 2 исправлены инлайн, см.
  `WORKLOG.md` раунд 3.)
- **Manual checks — статус честно:** браузерная проверка выполнена Chromium/Playwright на 8 размерах
  со скриншотами и замерами, включая ландшафтный телефон. Живой прогон в DevTools device toolbar
  пользователем и проверка на реальном устройстве с вырезом НЕ выполнялись — safe-area в эмуляции
  даёт нулевые инсеты, поэтому подтверждено только «ничего не ломает» (skeptic дополнительно
  проверил раскладку подстановкой ненулевых инсетов — выдерживает).
- **Amendment 13 (2026-07-19, при старте Step 15 — до реализации).** Причина: разбирая входные данные
  шага, я нашёл прямое противоречие ВНУТРИ утверждённого плана и вынес его пользователю до правок.
  AC1 требует, чтобы «overview-сцена **и** сцена отдела кадрировались корректно на Mobile (375px)», а
  finding 1 требует убрать с mobile загрузку overview-сцены как бесполезную. Оба верны только при
  разных ответах на вопрос «есть ли на mobile обзор офиса вообще». Фактически на ≤767px
  `OfficeSemanticMap` скрыт целиком (`display: none`), обзор — только карусель, при этом `docs/03`
  «Mobile» («**обзор**, карусель/пейджинг…») и `docs/08` («3. обзор офиса») обзор описывают. То есть
  документы обещали мобильный обзор, реализация Step 7 его не доставила, а Step 15 унаследовал
  противоречие.
  - Old scope: Step 7 / OQ-M5 — на ≤767px карта скрыта полностью, overview = только карусель;
    Step 15 finding 1 — устранить неиспользуемую загрузку `overview-768.avif` (~47 КБ).
  - New scope: на ≤767px мастер-сцена **показывается** над каруселью, с теми же пятью хотспотами
    поверх неё (тапаемыми). Загруженные 47 КБ перестают быть тратой впустую — finding 1 закрывается
    не удалением загрузки, а тем, что она становится осмысленной. Карусель остаётся основным
    touch-путём: только она показывает `overviewProblem` полностью (на хотспоте ≤767px описание
    свёрнуто — места нет), и она же остаётся целью focus-restoration, когда карта не видна.
  - Impact: `OfficeSemanticMap.module.css` (снятие `display: none`, кадр от ширины, а не от высоты),
    `DepartmentHotspot.module.css` (пол 44px + мобильная типографика подписи). Меняются утверждения
    двух существующих e2e, и это именно смена ожидаемого поведения, а не ослабление проверки:
    `mobile-touch-flow.spec.ts` («карта не в дереве доступности» → «карта видима, карусель тоже») и
    её же проверка focus после `ACTIVATE_CTA` (карточка карусели → первый хотспот, так как карта
    теперь идёт раньше в DOM); `tablet-touch-flow.spec.ts` AC11 (на 767px карта больше не скрыта).
    `docs/03` «Mobile» и `docs/08` правятся честно под факт.
  - Риск, принятый осознанно: на 375px кадр 3:2 даёт ~327×218, и самая мелкая зона (support,
    35%×18%) без пола была бы 114×39px — ниже тап-таргета. Пол `min-width/min-height: 44px` на
    `.hotspot` вводится глобально и закрывает заодно finding 2 (1280×400).
  - User approval: да, 2026-07-19 (выбран вариант «Сцена над каруселью + интерактивные зоны» из трёх
    предложенных; вариант был помечен мной как не рекомендуемый именно из-за пересмотра OQ-M5 —
    пользователь выбрал его сознательно).
- **Решение по finding 4 (2026-07-19, тот же раунд).** Перекрытие сцены карточками на не-desktop
  решается на планшете (768–1279): `.experience` получает `max-width`, как на desktop, чтобы полоса
  сцены гарантированно читалась. На ≤767px сцена отдела сознательно остаётся ФОНОМ — ширины там
  физически нет, — и это записывается в `docs/08` явно, а не умалчивается. (Отклонены: «оставить
  фоном на всём <1280px» и «ужать и на мобильном тоже».)
...
- Acceptance criteria:
  ...
  3. Браузер на узких ширинах грузит меньшую производную (проверка served-ширины через `sizes`), не
     desktop-размер.
  4. Весь e2e-набор (+ новые на tablet/mobile) и весь unit-набор зелёные; axe 0 serious на обеих
     ширинах — **для
     ...
     прежние тесты не ослаблены. **(Правка Step 15, раунд 2.)** Абсолютные числа «101 e2e / 133
     unit» из первой редакции удалены: они устарели ещё до старта шага (фактически на входе было
     ~137 e2e / 168 unit) и в таком виде проверяли не «набор зелёный», а «набор не менялся».
     Существенная часть критерия — «прежние тесты не ослаблены» — сохранена. Факт на закрытие
     шага: 177 e2e и 168 unit зелёные против production-сборки.
- Verification commands: `npm run format:check`, `npm run lint`, `npm run typecheck`, `npm run test`,
  `npm run build`, `npm run test:e2e`.
...
- Status: `COMPLETED` (skeptic Phase B: раунд 1 `FAIL` — обратимое CSS-условие гасило уже показанные
- **Приёмка в части блока кнопок пересмотрена Amendment 17** (Step 17, 2026-07-20): кнопки
  видимы с первого кадра и в каскад не входят. AC про «четыре блока последовательно» читать с этой
  поправкой — каскад остался у трёх блоков.
  блоки при уходе фокуса; раунд 2 оборван лимитом сессии дважды без вердикта; раунд 3 `PASS` со
  свежим агентом, все AC подтверждены независимыми замерами против production-сборки. Пять
  non-blocking находок исправлены инлайн, см. `WORKLOG.md`.)
- **Amendment 14 (2026-07-19).** Новый шаг, которого не было в milestone. Причина: прямой запрос
  пользователя со скриншотом — после выбора отдела четыре блока должны появляться друг за другом с
  задержкой 1 с (заголовок+проблема → список болей → окно пояснения → ряд кнопок), а при выборе
  другой боли внутри уже открытого отдела остальные блоки остаются видимыми и переоткрывается только
  окно пояснения, тоже через 1 с. Нумерация дробная (15.5) по прецеденту Steps 7.2–7.6 и 12.7 —
  вставка между шагами, Step 16 не переименовывается.
- Dependencies: Step 13 (`COMPLETED`, компоновка экрана отдела), Amendment 12 (`TypedText`,
  геометрия окна пояснения), Step 15 (`COMPLETED`, кадрирование — блоки анимируются поверх сцены).
- **Отступление от буквальной формулировки запроса, сделанное сознательно.** Четвёртый блок — это
  CTA и «Закрыть». Их появление через 3 с буквально нарушает три записанных правила: `CLAUDE.md`
  «Critical content must not depend on animation completion» и «CTA remains easy to reach», `docs/07`
  «критические кнопки после длинной анимации». Поэтому анимируется ТОЛЬКО ВИДИМОСТЬ: весь контент
  присутствует в DOM и в дереве доступности с первого кадра (тот же приём и та же причина, что у
  `TypedText`, Amendment 12). Визуально пользователь получает ровно запрошенный каскад.
- In scope:
  - Каскад видимости 0/1/2/3 с для четырёх блоков на Desktop/Tablet и его мобильный эквивалент
    (заголовок → аккордеон → кнопки).
  - Перезапуск каскада при открытии отдела И при переключении между отделами.
  - Выбор другой боли: остальные блоки не трогаются, окно пояснения переоткрывается через 1 с
    (печать текста стартует после задержки, а не под невидимым блоком).
  - `prefers-reduced-motion: reduce` — каскада нет вовсе, всё видно сразу.
  - Клавиатура: вход фокуса в панель мгновенно завершает каскад — иначе можно сфокусировать ещё
    невидимую кнопку.
  - Правка глобального reduced-motion-блока: он обнуляет `animation-duration`, но НЕ
    `animation-delay`, поэтому без правки именно пользователь с reduced motion ждал бы 3 с пустоты.
- Out of scope:
  - Переход overview↔отдел (Step 16), motion самой сцены, изменение текстов и геометрии блоков.
- Acceptance criteria:
  1. После выбора отдела четыре блока становятся видимыми последовательно с шагом ~1 с.
  2. Весь текст и все контролы присутствуют в DOM и в дереве доступности с первого кадра; CTA и
     «Закрыть» кликабельны и достижимы клавиатурой до завершения каскада.
  3. Выбор другой боли не перезапускает каскад остальных блоков; окно пояснения появляется через ~1 с.
  4. При `prefers-reduced-motion: reduce` каскада нет — все блоки видны сразу, без задержек.
  5. Прежние тесты не ослаблены; весь набор зелёный.
- Verification commands: `npm run format:check`, `npm run lint`, `npm run typecheck`, `npm run test`,
  `npm run build`, `npm run test:e2e`.
- Risks:
  - Задержки могут замедлить или расшатать существующие e2e (actionability ждёт стабильности бокса).
    Смягчение: анимировать только `opacity`, не трогая геометрию — бокс не движется.
  - Каскад может не перезапуститься при переключении отдела, если элементы не перемонтируются.
    Смягчение: `key` по `department.id` + проверка e2e именно на переключении, а не только на открытии.
- Rollback: `git revert` коммита(ов) шага — каскад аддитивен, снятие возвращает мгновенный показ.
- Профильный ревьюер: motion-engineer, qa-reviewer.

---

## TODO вне Step 15 — переводы строк ломают гейт `format:check`

- Status: `COMPLETED` (2026-07-20, отдельная задача пользователя; см. `WORKLOG.md`) (заведён 2026-07-19 по находке skeptic Phase B Step 15, NON-BLOCKING-6).
- Проблема шире одного шага: `core.autocrlf=true` при отсутствии `.gitattributes`. Prettier
  сконфигурирован с умолчанием `endOfLine: "lf"`, поэтому любой файл, прошедший через git-развёртку
  (в том числе через `git stash` round-trip, как в Steps 14 и 15), становится CRLF и валит
  `format:check` без единого изменения в содержимом. Сейчас красных файлов 8; **на свежем клоне git
  развернёт LF→CRLF во всех файлах, и гейт станет красным целиком.** То есть его текущая
  «почти зелёность» — локальная случайность, а не свойство репозитория.
- Варианты решения: `.gitattributes` с `* text=auto eol=lf` (и разовая нормализация индекса) либо
  `endOfLine: "auto"` в `.prettierrc`. Первый предпочтительнее — он фиксирует инвариант в
  репозитории, а не подстраивает проверку под сломанное состояние.
- Почему не сделано внутри Step 15: правка затрагивает файлы, к шагу отношения не имеющие
  (`CLAUDE.md` «Do not rewrite unrelated files»), и меняет содержимое рабочего дерева целиком —
  такое нельзя подмешивать в шаг про кадрирование.

---

## Step 16 — Motion overview↔отдел (контролируемое приближение)

- Status: `COMPLETED` (skeptic Phase B: раунд 1 `FAIL` — шаг сменил DOM-инвариант «сцена ровно одна»
  и молча ослабил приёмочный тест Step 13; раунд 2 `PASS`, три полных прогона против
  production-сборки, включая один под повышенной конкуренцией. Non-blocking исправлены инлайн.)
  **Переоткрывался Amendment 15** (2026-07-20) после того, как Manual checks, выполненные
  пользователем, вернули дефект. Объём Amendment 15 закрыт независимым skeptic-ревью за три раунда
  (раунд 2 — `FAIL`: внесённая мной правка производительности `will-change` не подтвердилась
  контролируемым A/B и была отменена; раунд 3 — `PASS`).
  **ЗАКРЫТ ОКОНЧАТЕЛЬНО 2026-07-20** после повторного ручного прохода пользователя на prod-сборке
  (`http://localhost:3200`): «теперь мне нравится, закрывай Step 16». Это и было последним
  недостающим условием — субъективная часть жалобы («выглядит дёшево») автоматическими проверками
  не закрывается в принципе, и подтвердить её мог только пользователь.
- **Manual checks — ВЫПОЛНЕНЫ пользователем 2026-07-20 и вернули дефект.** Ранее числились долгом;
  desktop-проход открыть/переключить/закрыть проведён пользователем и дал две находки: переход
  «не плавный, видны подёргивания, не всегда но часто» и «выглядит как дешёвое решение». Обе
  разобраны в Amendment 15 ниже — первая оказалась измеримым дефектом порядка событий, а не
  субъективной оценкой. Это ровно тот случай, ради которого Manual checks и помечены «обязательно»:
  ни один из 196 автоматических тестов дефекта не видел.
  **Повторный проход после Amendment 15 выполнен пользователем на prod-сборке 2026-07-20 —
  претензий нет, шаг закрыт.**
  Остаются НЕвыполненными и переносятся как известный долг без владельца: DevTools reduced-motion
  (поведение покрыто AC2-тестом, но глазами не смотрели) и **DevTools Performance** — профиль не
  снимался ни разу. Вместо него есть покадровый rAF-замер (`WORKLOG.md`, раздел Amendment 15), и он
  профиль не заменяет: два моих замера подряд оказались недостоверными именно из-за среды, а
  профиль DevTools снимается на машине пользователя и такой ошибки не допускает.
- **Amendment 15 (2026-07-20, по итогам ручной проверки пользователем — Manual checks выше
  ВЫПОЛНЕНЫ пользователем и вернули дефект).**
  - Status: `APPROVED` (прямой запрос пользователя).
  - Reason: пользователь посмотрел переход глазами и сообщил: «смена происходит не плавно, видны
    подёргивания, не всегда но часто» и «выглядит как дешёвое решение». Замер вскрыл причину, и это
    не вкусовщина, а **дефект принятой реализации**: уходящий слой снимается по `onReady`
    (событие загрузки), а анимация приходящего отсчитывается от МОНТИРОВАНИЯ — два независимых
    таймера. Измерено на dev-сервере, непрозрачность приходящей сцены в момент снятия уходящей:
    задержка сети 0 мс → **0.00**, 120 мс → 0.48, 250 мс → 0.77, 400 мс → 0.90, 600 мс → 0.99.
    То есть при картинке из кэша (а при повторных переключениях она всегда из кэша) переход
    проходит **через пустоту** — ровно тот дефект, против которого делался шаг. Step 16 закрыл его
    на медленном канале и открыл на быстром; e2e-тест ставил задержку 900 мс и щупал единственный
    режим, где дефекта нет.
  - Previous scope: приходящая сцена анимируется с момента монтирования (`scale(1.04)→1` +
    `opacity 0→1`, 700 мс); уходящая снимается по `onReady`.
  - New scope: (1) анимация приходящей сцены **стартует по факту готовности к отрисовке**
    (`img.decode()`, а не `onLoad`), а уходящая снимается **после того, как приходящая стала
    непрозрачной**, — два таймера сводятся в один причинный порядок; (2) сам эффект переработан на
    «кинематографический наезд с расхождением планов»: приходящая `scale(1.10)→1` за 800 мс по
    expo-out с отдельным, более коротким проявлением (420 мс), уходящая одновременно продолжает
    уходить `scale(1)→1.045`, плюс однократный мягкий световой пробег. Только `transform`/`opacity`.
  - Impact: длительность 800 мс остаётся внутри вилок `docs/07` (overview→department 650–1000,
    switch 450–800) — правка `docs/07` не требуется. Приёмочные критерии AC1–AC3 шага не
    ослабляются. `blur()`/`filter` осознанно НЕ вводятся: главная жалоба — плавность, а фильтр на
    полноэкранном фотослое её бы ухудшил. Требуется новый регрессионный e2e-тест на **быстрый**
    путь (провала яркости нет ни при какой скорости загрузки) — его отсутствие и есть причина,
    по которой дефект доехал до пользователя.
- **Входные данные из Step 15.5 (skeptic Phase B раунд 3, hidden risk — bookkeeping, не Amendment):**
  Каскад появления блоков (0/1/2/3 с) проигрывается заново на КАЖДОМ переключении отдела поверх
  `.switching`-фейда, поэтому CTA становится видимым примерно через 3.4 с после переключения. Это
  запрошенное пользователем поведение и оно в scope Step 15.5, но Step 16 накладывает на тот же стык
  свой переход 450–800 мс. Совокупный бюджет проверяется ЗДЕСЬ: критический контент обязан
  оставаться доступным (кликабельным и достижимым с клавиатуры) всё это время, а не «после ~4 секунд».
...
## Amendment 16 — подготовительные шаги перед Этапом 3 (по итогам milestone review Этапа 2)

- Status: `APPROVED`
- Reason: milestone review Этапа 2 (четыре независимых ревьюера, `WORKLOG.md`, раздел «Milestone
  review «Этап 2 — Art direction»») вернул Blocker=0, Critical=0, но выявил набор дефектов **на
  стыках шагов**, которые не видел ни один пошаговый skeptic-review, потому что каждый шаг
  проверялся изолированно. Три ревьюера независимо сошлись на одних и тех же швах. Часть находок
  (реестр разделов, каталог сцен, связка «сцена = отдел») — прямые препятствия Этапу 3: калькулятор
  и сцена «до/после» упрутся в них немедленно, и после Этапа 3 правка станет дороже.
- Previous scope: после Step 16 milestone «Этап 2» закрывается и начинается Этап 3.
- New scope: перед Этапом 3 выполняются два подготовительных шага — Step 17 и Step 18 ниже.
- Impact: не меняет ни целей Этапа 2, ни объёма Этапа 3. Новой функциональности не вводит: только
  устранение измеренных дефектов и подготовка точек расширения. Разбито на два шага намеренно —
  пять пунктов одним проходом не образуют независимо проверяемую единицу.
- User approval: **получено** 2026-07-20 («оформляй и приступай»), после предъявления сводного
  результата milestone review и двух находок motion-ревью, пришедших уже после решения
  пользователя по каскаду.

### Решение пользователя, зафиксированное отдельно

Каскад появления блоков (задержки 0/1/2/3 с, CTA видна через ~3.4 с, проигрывается заново на каждом
переключении отдела) — **оставить как есть** (решение 2026-07-20). Это принятое поведение, а не долг.
Step 17 не меняет тайминги каскада; он устраняет только **невидимую зону попадания** (пункт 3), что
решения пользователя не касается: оно относилось ко времени показа, а не к кликабельности
невидимого элемента.

## Step 17 — швы поведения (фокус, невидимая мишень, защёлка, стабильность приёмки)

- Status: `COMPLETED` (skeptic: раунд 1 `BLOCKED` — AC2 невыполним внутри scope, AC3 наполовину;
  раунд 2 `PASS`, блокирующих нет, переход к Step 18 разрешён. Восемь non-blocking исправлены инлайн.)
- **Amendment 17 (2026-07-20) — пересмотр приёмки Step 15.5 в части блока кнопок.**
  - Status: `APPROVED` (прямой выбор пользователя).
  - Reason: AC2 Step 17 («ни один блок каскада не принимает указатель, пока невидим») оказался
    невыполним внутри объёма: он противоречит приёмке Step 15 и Step 16 AC3 («критический контент
    доступен во время анимации», `CLAUDE.md` Motion rules). Оба технических лекарства **отвергнуты
    замером, а не мнением**: `pointer-events: none` роняет 4 теста Step 15/Step 16 по существу;
    приглушённый старт вместо невидимого роняет axe — контраст 1.29:1 при opacity 0.4 и 3.6:1 даже
    при 0.85 против нормы 4.5:1, потому что CTA проходит контраст с малым запасом и ЛЮБАЯ
    прозрачность выводит её за WCAG AA.
  - Previous scope (Step 15.5): блок кнопок — последняя ступень каскада, `animation-delay` 3 с на
    desktop и 2 с на mobile, до этого момента `opacity: 0`.
  - New scope: **блок кнопок видим с первого кадра**; каскад сохраняется у трёх остальных блоков
    (заголовок → боли → пояснение) с прежним расписанием. Анимация у блока кнопок оставлена как
    ЯКОРЬ ВРЕМЕНИ (`staged-hold`, from/to одинаковы): на её `animationend` держится защёлка
    `data-cascade-done`, без которой разваливается починка «текст ответа гаснет при ресайзе».
  - Impact: изменяет ранее утверждённое поведение Step 15.7/15.5 — поэтому оформлено Amendment, а не
    правкой тестов. Два теста Step 15.5 обновлены под новое поведение и **усилены**: добавлен
    инвариант «кнопки видимы сразу», а ожидание конца каскада перенесено на пояснение — последнюю
    реально анимируемую ступень. Решение пользователя «каскад оставить как есть» (Amendment 16)
    сохранено для трёх остальных блоков.
  - User approval: **получено** 2026-07-20 — выбор «Кнопки видны сразу, каскад у остальных трёх
    блоков» после предъявления замеров, закрывших два предыдущих варианта.
- Прежний статус раунда 1: `BLOCKED` — skeptic вернул два блокирующих: (1) AC2 не выполнен и **не выполним внутри
  утверждённого scope**: он противоречит приёмке Step 15/Step 16, и разрешить противоречие может
  только решение о самом каскаде, зафиксированном отдельным решением пользователя; (2) AC3 выполнен
  наполовину — `.gainTextReveal` наследует тот же дефект перезапуска анимации через `display`, а мой
  сторож его не ловит по построению (не выбирает боль и меряет `<p>`, а не анимируемый `<span>`).
  AC1 и AC4 выполнены и подтверждены независимо. Ожидает решения пользователя.
- Objective: устранить четыре измеренных дефекта на стыках шагов, не затрагивая архитектуру.
- Dependencies: Step 16 (`COMPLETED`).
- In scope:
  1. **Возврат фокуса при закрытии «Ваша задача»** (QA Major-1, нарушение `docs/11`). Сейчас
     `OfficeMachine.tsx` ищет `hotspot-<id>`, которого для `task` не существует, и фокус падает на
     `<body>` на desktop/tablet; на mobile работает случайно (кандидат — карусель, видимая только
     там). Плюс вынести контракт id целей фокуса в один типизированный модуль: архитектор описал
     это как класс из пяти неявных строковых связок между машиной и презентационным слоем.
  2. **Невидимая, но кликабельная CTA** (motion Major 2). У `.stageActions` `opacity: 0` на время
     задержки при `pointer-events: auto` — измерено, на t=300 мс бокс ссылки доступен указателю.
     Тайминги каскада НЕ меняются.
  3. **`.gainInitial` в одностороннюю защёлку** (motion Major 3). Переключение `display` по
     медиазапросу перезапускает анимацию с `backwards`-fill, и после отыгранного каскада ресайз
     через 768px гасит окно результата на 2.4 с. Триггеры бытовые: поворот планшета, зум, devtools.
  4. **Стабильность приёмки** (QA Major-2 + архитектор M8, m5). e2e даёт до 4 падений из 196 на
     холодном сервере под полной параллельностью (`page.goto` timeout), при серийном прогоне 30/30
     зелёные. Дополнительно: `playwright.config.ts` с `reuseExistingServer` на порту 3100 может
     молча прогнать «prod»-гейт против dev-сервера — это остаточный риск, записанный без владельца
     ещё в Amendment 1. Закрывается выделенным портом для e2e.
- Out of scope: тайминги каскада; архитектурные правки (Step 18); Step 12.7 и форма.
- Acceptance criteria:
  1. Закрытие «Ваша задача» возвращает фокус на открывший контрол на всех трёх классах ширины;
     e2e утверждает это явно (сейчас тест закрытия существует, но фокус не проверяет).
  2. Ни один блок каскада не принимает указатель, пока невидим; клавиатурный путь не страдает.
  3. После отыгранного каскада ресайз через 768px в обе стороны не гасит окно результата.
  4. Полный e2e зелёный на холодном сервере под полной параллельностью, повторяемо; гейт не может
     быть прогнан против dev-сервера.
- Rollback: `git revert` коммита шага; правки аддитивные и локальные.

- **Amendment 18 (2026-07-20) — фактическая граница AC3 Step 18.**
  - Status: `APPROVED` (прямой выбор пользователя).
  - Reason: AC3 требовал, чтобы и седьмая сцена, и седьмой раздел добавлялись «не более чем в одной
    точке каждое». Для сцены это достигнуто. Для раздела — нет, и упереться в это можно было только
    выполнив шаг: реестр описывает ДАННЫЕ раздела, но не его содержимое, а положить в него
    React-компонент значило бы затащить UI в слой машины состояний — ровно ту связанность, ради
    устранения которой реестр и вводился. Найдено skeptic-ревью (раунд 1, N1; раунд 2, находка 5),
    а не заявлено исполнителем.
  - Previous scope (AC3): «Добавление седьмого раздела и новой сцены не требует правок более чем в
    одной точке каждое».
  - New scope (AC3): **новая СЦЕНА — одна точка** (строка в `scripts/office-scenes.config.mjs` +
    `npm run assets:scenes`); **новый ОТДЕЛ — ноль правок кода** (данные `data/departments.json` и
    `data/office-zones.json`), с оговоркой, что это было верно и до шага; **новый раздел ИНОГО
    рода — запись в реестре плюс проброс типа, адреса и содержимого** (`OfficeSectionId` в
    `reducer.ts`, разбор адреса в `url-sync`, тернарник выбора содержимого в `OfficeExperience`,
    объединение `OfficeSectionKind` и сам компонент). Граница записана в док-комментарии
    `src/features/office-machine/sections.ts` и обязана оставаться синхронной с этим пунктом.
  - Impact: снижает формальное требование AC3 в части разделов; НЕ отменяет достигнутого. Реальный
    выигрыш реестра сохраняется и именно он был целью пункта 4 in scope: устранена четвёртая копия
    знания о составе разделов в `focusTargets` (та самая, которую Step 12.7 забыл обновить, отправив
    фокус на `<body>` на desktop и tablet) и дедуплицирована разметка пункта рельса.
  - User approval: **получено** 2026-07-20 — выбор «Признать фактическую границу» после предъявления
    трёх вариантов, включая расширение scope до настоящей одной точки.

## Step 18 — точки расширения под Этап 3 (слой сцены, реестр разделов, каталог сцен)

- Status: `COMPLETED`
- Приёмка: skeptic раунд 1 — `FAIL` (три блокирующих: перенесённая привязка к шести сценам,
  наполовину названная цена `sizes`, гарантия, обоснованная несуществующим тестом); раунд 2 —
  `PASS`, блокирующих нет. AC3 закрыт **Amendment 18** выше, а не заявлением о выполнении.
- Objective: убрать препятствия, в которые Этап 3 упрётся немедленно, и закрыть последний
  измеренный случай перехода через пустоту.
- Dependencies: Step 17.
- In scope:
  1. **Удержание сцены на пути overview→отдел** (архитектор M1 + motion Major 1 — сошлись
     независимо). `SceneCrossfade` живёт ВНУТРИ ветки активного раздела и при открытии монтируется
     заново, удерживать ему нечего. Измерено: 89–138 мс белой сцены на localhost, 378/759/1444 мс
     при задержках 300/600/1200 мс, **863 мс на мобильном Fast 3G**. Это тот же дефект, ради
     которого делался Amendment 15, на другом конце пути.
  2. **Сторож на этот путь.** Все три существующих теста укрытости входят через `?department=<id>`
     и переключаются рельсом, то есть проверяют только путь отдел→отдел — дефект не виден им **по
     построению**. Отсутствие сторожа и есть причина, по которой он дожил до конца этапа.
  3. **Правка док-комментария `SceneCrossfade.tsx`**, утверждающего инвариант «ни в одной точке
     цепочки экран не может остаться без кадра» шире, чем он выполняется.
  4. **Реестр разделов** вместо бинарной ветки 90%-области и литерального шестого пункта рельса
     (архитектор M7). Сегодня новый раздел добавляется вручную в шести точках; калькулятор Этапа 3
     повторит все шесть.
  5. **Каталог сцен** (архитектор M6): сейчас продублирован в генераторе, в 36 ручных импортах и в
     тесте, фиксирующем ровно шесть сцен, — сцена «до/после» сначала уронит зелёный тест.
- Out of scope: новая функциональность Этапа 3; устранение двух путей рендера сцены сверх того, что
  само собой закрывается подъёмом слоя; UX-находки (инверсия CTA в hero, пустота overview,
  мобильная двойная навигация, «Как это может работать») — они продуктовые и решаются отдельно.
- Acceptance criteria:
  1. Укрытость кадра на пути overview→отдел равна 1.00 при задержках сцен 0/300/900 мс; закреплено
     e2e-сторожем, который до правки падал бы.
  2. Инварианты Amendment 15 (переключение отдел→отдел, возврат до загрузки, провал загрузки,
     быстрое переключение) не регрессируют.
  3. Добавление седьмого раздела и новой сцены не требует правок более чем в одной точке каждое.
- Rollback: `git revert`; Step 18 аддитивен поверх Step 17.
...
- Границы milestone: пилот выполняется на ОДНОМ отделе — «Продажи» (`sales`). Этапы 4–7 переносят тот
  же шаблон на остальные четыре отдела, поэтому каждый механизм (до/после, калькулятор, analytics,
  графика сцены) строится ОБОБЩАЕМО (принимает данные отдела), а контентом подключается ТОЛЬКО
  «Продажи». Разовое решение под один отдел запрещено — оно провалит перенос на Этапах 4–7.
...
- Deliverable «copy» (`docs/13:17`) НЕ вынесен в отдельный шаг намеренно: деловая копия «Продаж»
  финализируется в тех шагах, где она рождается — `beforeSteps`/`automationSteps` (Step 19), тексты
  калькулятора и disclaimer (Step 22), копия CTA/подтверждения (Step 26), — и каждый из этих шагов
  несёт ux-strategist в ревьюерах. Отдельная приёмка «copy» не теряется: она распределена, а не
  пропущена (skeptic-ревью плана, N4).
...
- **OQ-3-C (гейт Step 22–23): реальная формула экономии и входы по умолчанию для «Продаж».** Какие
  входы, какая формула, какой disclaimer. Нужны реальные цифры пользователя.
- **OQ-3-A (гейт Step 24–25): транспорт аналитики + позиция по consent.** dev-sink/no-op |
  first-party cookieless `POST /api/analytics` | сторонний cookieless | сторонний с cookie (тянет
  consent Этапа 8). Подтвердить cookieless/no-PII для пилота.
- **OQ-3-D (гейт Step 26): адресат CTA пилота / нужен ли реальный on-site lead-capture.** Форма «Ваша
  задача» (Step 12.7, отложена на конец проекта) готовым путём лид-захвата НЕ считается.
...
- Строго последовательно: **19 → 20 → 21**; **22 → 23**; **24 → 25** (25 требует и 20, и 23).
- Цепочка «до/после» (19–21) и цепочка «калькулятор» (22–23) логически независимы, но обе
  интегрируются в экран «Продажи» — вести последовательно ради ограниченного diff и независимого
  skeptic-review, не сливать в один проход.
- Step 27 (кадр ≥1584px) без апстрим-зависимостей — в любой момент/параллельно.
- Step 26 зависит только от OQ-3-D; Step 28 — от 20 и 23; Step 29 — от всех UI-шагов.
...
## Step 22 — Движок калькулятора экономии (чистый модуль + формула)

- Status: `PROPOSED`
- **ГЕЙТ: OQ-3-C (формула и входы для «Продаж»)** — старт шага блокирован до ответа пользователя.
...
## Step 23 — UI калькулятора: панель внутри экрана «Продаж» (client) [user-facing]
...
## Step 24 — Слой аналитики: типизированный `trackEvent` + транспорт

- Status: `PROPOSED`
- **ГЕЙТ: OQ-3-A** решает ТОЛЬКО транспорт и позицию по consent. **Словарь событий и параметров НЕ
  открыт — он задан утверждённым `docs/15-analytics-conversion.md`** и является для этого шага
  источником истины (правка после skeptic-ревью плана, находка B1: первая редакция выдумала имена
  `department_opened`/`cta_clicked` и сослалась на `docs/09` — прямое нарушение «Never silently
  contradict approved documents»).
...
## Step 25 — Инструментовка событий пилота «Продажи» (по `docs/15`)
...
- Status: `PROPOSED`
- **ГЕЙТ: OQ-3-D (адресат CTA / нужен ли on-site lead-capture)** — старт блокирован до ответа.
- Objective: определить и реализовать назначение CTA пилота так, чтобы оно НЕ зависело от отложенной
  формы «Ваша задача» и НЕ обещало контакта, который не собирается.
- Dependencies: OQ-3-D. НЕ зависит от закрытия Step 12.7.
- In scope (по OQ-3-D):
  - (D-a) сохранить `contactHref` (Telegram) — шаг сводится к явной фиксации, что on-site лид-захвата
    в пилоте нет, копия ничего лишнего не обещает;
  - (D-b) минимальный on-site lead-capture, РЕАЛЬНО собирающий канал обратной связи, обобщаемый,
    честная копия без обещаний выручки/автономии;
  - (D-c) иной путь.
  - В любом варианте копия CTA/подтверждения приведена в соответствие с тем, что реально
    собирается/происходит.
  - **Конверсионное событие аналитики (перенесено сюда из Step 25, N1):** имя из `docs/15` по
    выбранному варианту — D-a (Telegram) → `contact_open`; D-b (on-site lead-capture) → `lead_submit`
    + `lead_submit_success`/`lead_submit_error`. Инструментуется здесь, потому что только этот шаг
    знает, что CTA делает.
  - Тесты под вариант (unit/e2e), axe.
- Out of scope: доработка формы «Ваша задача»; CRM/хранение заявок (Этап 8); лид-захват других отделов.
- Acceptance criteria:
  1. CTA пилота функционирует независимо от статуса Step 12.7.
  2. Копия не обещает того, что не собирается/не делается.
  3. Путь достижим клавиатурой, фокус виден, axe 0 serious/critical.
  4. Конверсионное событие носит имя из `docs/15` по выбранному варианту (`contact_open` или
     `lead_submit*`) и наблюдаемо в e2e.
- Verification: `format:check`, `lint`, `typecheck`, `test`, `build`, `test:e2e`.
...
- Status: `PROPOSED`
- Objective: тяжёлые части пилота (калькулятор, до/после, графика сцены) не грузятся и не
  анимируются, пока не нужны (`CLAUDE.md` Performance rules).
- Dependencies: Step 20, Step 23 (и Step 21).
- In scope: ленивое подключение калькулятора/до-после (dynamic import где оправдано); пауза
  несущественной анимации при скрытии/`VISIBILITY_CHANGE`; нет React-обновлений на каждый pointer
  move; трансформации/opacity вместо layout thrashing. Тесты/замеры.
- Out of scope: изменение поведения фич; perf других отделов.
- Acceptance criteria:
  1. Ассеты калькулятора/до-после не запрашиваются, пока пользователь их не достиг.
...
- Status: `PROPOSED`
- Objective: единая проверка a11y и reduced-motion для СОБРАННОГО пилота (сцена + до/после +
  калькулятор + CTA), закрывающая требование Этапа 3 «accessibility».
- Dependencies: Steps 20, 21, 23, 25, 26.
- In scope: расширить `accessibility-scan.spec.ts` и reduced-motion-тесты на полный пилот. Полный
  ...
  **screen reader states** (объявляемые статусы динамических блоков — до/после, калькулятор), контраст,
...
- Acceptance criteria:
  ...
  5. Динамические статусы (до/после проигран, расчёт обновлён) объявляются скринридеру; при zoom 200%
     пилот остаётся работоспособным без потери контента (`docs/14`).
- Verification: `format:check`, `lint`, `typecheck`, `test`, `build`, `test:e2e`.
...
главную цель milestone: механизмы (до/после, калькулятор, analytics, графика) обобщаемы и готовы к
переносу на Этапы 4–7, а не разовые под «Продажи».
