...
Затем отправьте текущие URL из production-контейнера. **Сначала всегда сухой прогон** — он
выполняет все проверки и не отправляет ничего:

```bash
docker compose exec allqbit-site npm run indexnow:submit-current -- --dry-run
```

Проверьте в выводе `submitted: false`, число `canonicalCount` и сам список адресов. Если всё
верно — тот же запуск без флага выполняет отправку:

```bash
docker compose exec allqbit-site npm run indexnow:submit-current
```

Скрипт проверяет по порядку и останавливается на первом отказе, **не обращаясь к IndexNow**:
конфигурацию, TXT-файл подтверждения, код ответа и тип содержимого `sitemap.xml`, затем состав
карты сайта, затем шесть постоянных legacy-редиректов. Состав карты проверяется семантически, а
не по числу адресов: обязаны присутствовать восемь код-роутов (`/`, `/products`, `/documents`,
`/how-we-work`, `/faq`, `/contacts`, `/blog`, `/cases`), все пять `/solutions/*` и хотя бы по
одному материалу в `/products/`, `/blog/` и `/cases/`; запрещены чужой host, не-https, параметры
запроса, фрагменты, `/admin`, `/api`, `/login`, `/departments/*`, `/solutions` и
`/solutions/executive`. Дубликаты не являются ошибкой — они удаляются, а их число попадает в отчёт
как `duplicatesDropped`.

Поэтому публикация или снятие материала в админ-панели **не требует правки кода**: число адресов в
карте меняется законно, и скрипт это допускает. Отказ означает настоящую поломку, а причина
печатается полем `reason` вместе с подробностями. Ключ в вывод не попадает никогда.

Единственное, что требует отдельной задачи, — сознательное снятие отдела с публикации: все пять
`/solutions/*` сейчас обязательны.
