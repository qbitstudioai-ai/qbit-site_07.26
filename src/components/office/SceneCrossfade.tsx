 ...
 *
 * ГРАНИЦА ЭТОГО ИНВАРИАНТА (уточнено в Step 18). Прежняя редакция комментария утверждала: «ни в
 * одной точке этой цепочки экран не может остаться без кадра». Утверждение шире, чем выполняется, и
 * широкая формулировка не безобидна — именно она позволяла считать путь overview→отдел закрытым,
 * пока он был открыт: этот компонент удерживает предыдущий кадр только там, где ЖИВ САМ, а он тогда
 * монтировался внутри ветки активного раздела и при открытии отдела начинал с пустого стека.
 * Дефект закрыт снаружи — подъёмом слоя в OfficeExperience, а не здесь.
 *
 * Точная формулировка: пока компонент смонтирован, СМЕНА сцены не обнажает экран. Остаётся ровно
 * один случай без предыдущего кадра — ПЕРВОЕ монтирование (первая загрузка страницы), когда
 * удерживать нечего по определению. Там пустоту закрывает не этот компонент, а фон кадра
 * (`.stage`, --color-graphite-100) и плейсхолдер Step 8.
 *
