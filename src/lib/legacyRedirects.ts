 ...
 * шести неизменных адресов добавило бы работу на каждый запрос и втянуло бы SEO-переадресации в
...
export const LEGACY_REDIRECTS: readonly LegacyRedirect[] = [
  ...
  /**
   * `/examples` — прежний адрес раздела примеров работ. Раздел существует и отвечает 200 по адресу
   * `/cases`; сам `/examples` маршрута не имеет и до этой строки отдавал 404. Ссылок на него в
   * текущем контенте уже нет (это стерегут тесты блога) — переносится проиндексированный адрес,
   * а не действующая навигация.
   *
   * Получатель `/cases` — статический маршрут в коде (`src/app/cases/page.tsx`), а не slug из
   * админ-панели, поэтому оговорка выше про переименование в базе к этой паре не относится.
   */
  { source: "/examples", destination: "/cases" },
];
