...
test.describe("office overview redesign", () => {
   ...
   *
   * Отсюда два правила для этого списка. Первое: обе стороны каждого рубежа — 1023/1024 (бургер →
   * строка) и 1359/1360 (компактный набор → полный). Второе, важнее: ширины ВНУТРИ бывшей
   * сломанной полосы (1200, 1233) — граничные значения сами по себе дефект не ловят, проверено.
   *
   * Оба рубежа сдвинуты вверх 2026-08-10 вместе с ДЕСЯТЫМ пунктом меню («Кейсы»): компактному
   * набору десяти пунктов нужно ≥1008px вместо прежних 945px, полному — ≥1332px вместо 1235px
   * (замер с шагом 4px на production-сборке, объяснение и цифры — в `Header.module.css`). Прежние
   * 960 и 1280 переехали в бургер и в компактный набор соответственно; в списке они оставлены —
   * теперь это ширины ВНУТРИ новых полос, а именно такие значения и ловили дефект прошлый раз.
   *
  ...
  test("the desktop header does not overflow at style-set boundaries", async ({ page }) => {
     ...
     *
     * Замерено на ИСПРАВЛЕННОЙ раскладке, и главная здесь не худший случай: на 1024px поле равно
     * 20.6px на `/`, но 16.5px на `/documents` — у внутренней страницы один пункт меню несёт
     * `aria-current="page"` с `font-weight: 650`, и меню шире на 6–7px. Дальше поле только растёт:
     * 24px на 1040…1359px, 22…39px на 1360px и шире. Цифры пересняты 2026-08-10 после десятого
     * пункта меню; сам порог MIN_RIGHT_GAP не менялся.
     *
    ...
    for (const route of ["/", "/documents"]) {
      for (const width of [960, 1023, 1024, 1180, 1200, 1233, 1280, 1300, 1359, 1360, 1366, 1440]) {
  ...
  /* Ниже 1024px шапка обязана быть свёрнута в бургер, выше — оставаться строкой. Рубеж проверяется
     с обеих сторон: именно на нём десять пунктов перестают помещаться в строку. Значение поднято
     с 960px 2026-08-10 вместе с пунктом «Кейсы» — замер в `Header.module.css`. */
  test("switches to the disclosure menu exactly at the 1024px boundary", async ({ page }) => {
    await page.goto("/");
    const menuButton = page.locator('button[aria-controls="site-navigation"]');
