...
/**
 * Заголовок главной для поисковой выдачи.
 *
 * Задаётся отдельно от видимого H1 намеренно. H1 (`headline`) — обещание человеку, уже открывшему
 * страницу; `<title>` — строка в выдаче, где важны запросы, по которым страницу ищут, и длина
 * сниппета. Раньше title собирался как `${copy.headline} — ${SITE_NAME}`, из-за чего эти две задачи
 * решались одной формулировкой. Бренд приписывает `withBrand`, чтобы название компании оставалось
 * в одном месте (`src/lib/seo.ts`).
 *
 * Описание при этом по-прежнему берётся из видимого текста (`subheadline`): там расхождения между
 * сниппетом и содержимым страницы не нужны.
 */
const HOME_SEO_TITLE = withBrand("ИИ-автоматизация продаж и бизнес-процессов");

/**
 * Метаданные главной.
 *
 * Описание берётся из УЖЕ показанного на странице текста (`subheadline` — видимый абзац под H1), а
 * не сочиняется заново: правка текста в админ-панели должна менять и метаданные, иначе они
 * разойдутся с содержимым. Заголовок — см. `HOME_SEO_TITLE` выше.
 *
