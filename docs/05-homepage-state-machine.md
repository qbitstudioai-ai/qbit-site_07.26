...
**Статус реализации (актуально после Step 3):** клиентское разделение `hero` → `ACTIVATE_CTA` →
`overview` для пользователей с включённым JavaScript **ещё не реализовано**. Текущий код (Step 3)
рендерит только вариант "без JavaScript", описанный выше, — единый статичный HTML, где все пять
отделов видны сразу, независимо от того, включён JavaScript или нет; основная CTA-кнопка не имеет
обработчика клика (`onClick`) и является no-op и с JS, и без него. Реальный переход `hero` →
`overview` по клику (раскрытие пяти отделов только после `ACTIVATE_CTA`) — задача Step 4
("Homepage state machine"). До завершения Step 4 это состояние документа описывает целевое
поведение, а не поведение уже работающего сайта.
