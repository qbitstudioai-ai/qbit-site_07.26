...
**Где мы находимся.** Закоммичено по `a4a45ea` включительно (Step 10 и Step 11). Всё, что сделано
после, лежит в рабочем дереве НЕЗАКОММИЧЕННЫМ: ~44 файла — Step 12, Amendments 8/9/10, Step 12.7.
Гейт на момент передачи зелёный: `format:check`/`lint`/`typecheck` PASS, **159 unit**, **114 e2e**,
`build` exit 0.

**Что делать в новой сессии, по порядку.**

1. **Разложить незакоммиченное дерево на коммиты** (рекомендация, пользователь ещё не подтвердил).
   Разумное деление: (а) Step 12 — сцена overview; (б) Amendment 9 — CTA в Telegram, фикс
   `:focus-visible`, брони под текст; (в) Amendment 10 — CTA отделов; (г) Step 12.7 — раздел «Ваша
   задача». Такое деление сохраняет revert-безопасность: откат раздела с формой не тронет сцену.
2. **Закрыть Step 12.** Пользователь подтвердил, что зоны попадают верно («Зоны попадают куда надо»),
   но просил не закрывать шаг сразу, пока вносились правки. Правки внесены. Нужен только его явный
   ответ → `PASSED` → `COMPLETED` + README.
3. **Step 13 — сцены отделов.** Главный визуальный шаг. ВНИМАНИЕ: объём пересмотрен **Amendment 8** —
...
4. Далее по плану: Step 14 (логотипная графика), Step 15 (адаптивное кадрирование), Step 16 (motion).
5. **В самом конце, по решению пользователя:** Step 12.7 (боты/отправка) — `skeptic` Phase B и
